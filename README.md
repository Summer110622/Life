...
4. 「旅をはじめる」でモデル取得を開始します。初回は数百MB規模の通信とGPUメモリが必要です。モデルを待たずに旅の選択は進められます。WebGPUが使えない場合はゲーム画面上部の「CPU」を選べます。
...
- 各回答へのLFMコメントは生成順に記録され、ゲーム画面右下の「LFMの言葉」から読み返せます。モデル準備中の回答は順番に待機します。
- タイトル画面とゲーム画面の言語ボタンで日本語／英語を切り替えられます。切り替え時に画面が再読み込みされ、回答は維持されます。言語の違うAI生成文は消去され、次の生成で選択した言語を使います。
...
- 最初の回答でLFM生成が始まり、時間がかかる環境でもコメントの逐次出力が吹き出しと記録欄に表示されることを確認。
