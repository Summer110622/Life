...
import { questions, characters, factorNames, sceneEpigraphs, archePhil } from '../story.mjs';
import { summarize, loadSave, buildPrompt } from '../game-core.mjs';
test('eighteen scenes, five renamed characters, seven transparent game indicators', () => {
  assert.equal(questions.length,18); assert.equal(characters.length,5); assert.equal(Object.keys(factorNames).length,7);
...
test('completed save restores and reasons are bounded', () => {
  const saved=loadSave(JSON.stringify({version:2,index:17,answers:Array(18).fill('B'),reasons:['x'.repeat(500)],completed:true,report:'手紙'}),questions);
  assert.equal(saved.completed,true); assert.equal(saved.report,'手紙'); assert.equal(saved.reasons[0].length,240);
});
test('scenes carry philosophical epigraphs and archetypes close with one', () => {
  assert.equal(sceneEpigraphs.length, 6);
  sceneEpigraphs.forEach(l => assert.ok(l.length > 5));
  assert.deepEqual(Object.keys(archePhil).sort(), Object.keys(factorNames).sort());
  Object.values(archePhil).forEach(l => assert.ok(l.length > 5));
});
